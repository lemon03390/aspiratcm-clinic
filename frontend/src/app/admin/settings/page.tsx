...
import { ChevronDownIcon, ChevronRightIcon, PlusIcon, PencilIcon, TrashIcon, ArrowDownTrayIcon, ArrowUpTrayIcon } from '@heroicons/react/24/outline';
...
export default function AdminSettingsPage() {
  ...
  const fetchSettings = async (category: string) => {
    setLoading(true);
    try {
      const response = await axios.get(`/api/v1/settings/${category}${searchText ? `?search=${searchText}` : ''}`);
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (editingRecord) {
        // 更新現有記錄
        await axios.put(`/api/v1/settings/${activeCategory}/${editingRecord.id}`, form);
        showToast('設定已更新');
      } else {
        // 新增記錄
        await axios.post(`/api/v1/settings/${activeCategory}`, form);
        showToast('設定已新增');
      }
  ...
  const confirmDeleteAction = async () => {
    ...
    setLoading(true);
    try {
      await axios.delete(`/api/v1/settings/${activeCategory}/${confirmDelete.id}`);
      showToast('設定已刪除');
      fetchSettings(activeCategory);
    } catch (error) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold">TCM 參考資料管理</h1>
        <p className="text-gray-600 mt-1">管理中醫辨證、病名、治則等參考資料</p>
      </div>
