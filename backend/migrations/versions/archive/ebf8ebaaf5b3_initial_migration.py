...
Revision ID: ebf8ebaaf5b3
Revises: 
Create Date: 2025-03-23 22:47:29.291071
...
def downgrade() -> None:
    """Downgrade schema."""
    # ### commands auto generated by Alembic - please adjust! ###
    def drop_table_with_indexes(table, indexes):
        for idx in indexes:
            op.drop_index(op.f(f'ix_{table}_{idx}'), table_name=table)
        op.drop_table(table)
    
    drop_table_with_indexes('appointments', ['patient_name', 'id'])
    drop_table_with_indexes('doctors', ['name', 'id', 'email'])
    # ### end Alembic commands ###
